...
- RViz2 の **Publish Point** で `/clicked_point` を publish すると、順番付きウェイポイントとして追加
- ウェイポイントは 0 個でも動作（従来どおり、`/goal_pose` を最終目標として使用）
- 推論開始トリガーは従来どおり `/goal_pose`（2D Goal Pose）
- 先頭の未到達ウェイポイントに一定距離まで接近すると、次のウェイポイントへ自動遷移
- 未到達ウェイポイントが無い場合は `/goal_pose` を参照して距離・角度を計算
...
| `/clicked_point` | `geometry_msgs/PointStamped` | RViz2 の Publish Point。ウェイポイントを順番に追加。 |
...
| `clicked_point_topic` | `/clicked_point` | ウェイポイント追加に使う PointStamped トピック名。 |
| `waypoint_reach_threshold_m` | `0.4` | 先頭ウェイポイントを到達済みと判定する距離しきい値（m）。 |
| `clear_waypoints_on_new_goal` | `false` | 新しい `/goal_pose` を受信した際に既存ウェイポイントをクリアするか。 |
